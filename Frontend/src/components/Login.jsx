...
<dialog id="my_modal_3" className=" modal ">
  <div className="modal-box dark:bg-slate-900 dark:text-white">
    <form onSubmit={handleSubmit(onSubmit)} method="dialog">
      ...
      <Link to='/' className="btn btn-md btn-circle btn-ghost absolute right-6 top-6"onClick={()=>
        document.getElementById("my_modal_3").close()
        }>✕</Link>
    
    <h3 className="font-bold text-lg right-6 top-6">{" "}Login</h3>
  ...
  <p>Not registered? <Link to="/signup" className='underline text-blue-500 curser-pointer'>SignUp</Link></p>
  </div>
  </div>
  </form>
  </div>
